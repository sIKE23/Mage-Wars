...
targetMenu() -- From main.py
	attackChoicePrompt()
		...
		chanceToKill
		changeForEffect
		createAttackOptionsList
		askChoice (API call)
	payForAttackSpell
		castSpell

Still need to add logic to apply and modify effects/effect rolls
...
def attackChoicePrompt(attacker,defender,actionFilters=["Quick","Full"]):
	...
	modifiedAttackList = [computeAttack(attacker,attack,defender) for attack in attackList]
	
	#4: Compute the kill chances for the modified attacks
	killChances = [chanceToKill(attacker,attack,defender) for attack in modifiedAttackList]

	#5: Compute Effect chances for the modified attacks
	effectChances = [chanceForEffect(attacker, attack, defender) for attack in modifiedAttackList]

	#6: Generate the list of choices that will appear on the menu
	choiceText, options, colors = createAttackOptionsList(modifiedAttackList, killChances, effectChances)
	
	#7: Display the menu to the player and allow them to choose an option
	choice = askChoice(choiceText, options, colors)

	#8: Initiate the chosen attack.
	...
	if choice > 0:
		index = choice - 1
		return(modifiedAttackList[index])
...
#Moved to rubbish.py just in case for now
