...
def resolveSirenHeal(traits, card):
	damageAmount = 0
	if "Finite Life" in traits and card.controller == me and card.isFaceUp:
		notify("{} has the Finite Life Trait and can not heal".format(card.name))
		return
	if me.Damage > 0:
		damageAmount = 1
		subDamageAmount(card, 1)
	if damageAmount > 0:
		notify("{} heals {} damage from the Aquatic Terrain ".format(me,damageAmount))
	else:
		notify("{}'s {} is already at full health".format(me, card.name))
