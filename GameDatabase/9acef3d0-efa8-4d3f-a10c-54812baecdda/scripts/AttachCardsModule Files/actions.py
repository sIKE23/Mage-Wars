...
def onMoveCard(player,card,fromGroup,toGroup,oldIndex,index,oldX,oldY,x,y,isScriptMove):
        ...
        if card.controller == me and fromGroup == table: #Does not trigger when moving cards onto the table
                ...
                if toGroup != table:
                        detachAll(card)
                if not (oldIndex != index and oldX==x and oldY==y) and not isAttached(card): #Do not realign if it is only the index that is changing. Prevents recursions.
...
def moveCard(model, x, y):
	for c in table:
		if c.model == model:
			c.moveToTable(x, y)
			alignAttachments(c) #ACG-Attachment code to move attachments as well
			return c
